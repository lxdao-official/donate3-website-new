    ...
    return <Layout bgColor='#fdfaf8'>
        <Box sx={{
            ...
            <Box sx={{
                ...
                {[{ title: 'Create', icon: '/icons/gear.svg', path: '/create' }, { title: 'Dashboard', icon: '/icons/dashboard.svg', path: '/dashboard' }].map((val, index) => {
                    return <MyTab
                        onClick={() => { router.push(val.path) }}
                        key={index} active={router.asPath.startsWith(val.path)} title={val.title} icon={val.icon} />
                })}
