...
export const convertIpfsToLink = (options: { ipfs: string; gateway: string; timeout?: number }, callback: (err?: Error, url?: string, response?: any) => void) => {
...
export const getFasterIpfsLink = (options: { ipfs: string; timeout?: number; gateways?: string[] }) => {
  ...
  return new Promise<Partial<ICustomWidget>>((resolve, reject) => {
    let tasks: Array<ReturnType<typeof convertIpfsToLink> | null> = [];

    function fetchCallback(opts: { index: number; gateway: string }, err?: Error, url?: string, response?: any) {
    ...
    tasks = taskGatewayList.map((gateway, index) => {
      return convertIpfsToLink({ ipfs, gateway, timeout }, (err, url, response) => {
